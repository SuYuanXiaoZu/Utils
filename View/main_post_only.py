...
if __name__ == '__main__':
    ...
    with open(f'{host}/OneHop.json', 'r') as FJ:
        with open(f"{host}/{dstIp}-postOnly.csv", "w", encoding="utf-8") as postOnly:
            with open(f"{host}/{dstIp}-testone.csv", "w", encoding="utf-8") as fullHttp:
                postOnly_writer = csv.writer(postOnly)
                fullHttp_writer = csv.writer(fullHttp)
                postOnly_writer.writerow(['HOST', 'TIME', 'HTTP', 'POST'])
                fullHttp_writer.writerow(['HOST', 'TIME', 'HTTP', 'POST'])
                frame = json.load(FJ)
                # 读取json文件内容
                for data in frame:  #
                    #print(d1)
                        ...
                        if http:
                            ...
                            if content[0] not in Filter_command:
                                ...
                                # if frame_time:
                                    ...
                                    if host == dstIp:
                                        fullHttp_writer.writerow(result)
                                        if content[0].startswith("POST"):
                                            postOnly_writer.writerow(result)
